...
def monthly_payment_vs_mortgage_chart():
	...
	for i in range(0,principal.shape[0]):
		monthly_payment[i] 	= (principal[i]-60000) * monthly_interest/(1-(1+monthly_interest) ** (-20*12)) - flat_income

	for i in range(0,principal.shape[0]):
		print str(principal[i]) + '\t' + str(3750 - monthly_payment[i])
